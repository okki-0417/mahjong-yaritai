...
export default function AverageRakingFormControls() {
  ...
  const averageRankingByParticipant = participantUserFields.map(
    (_, participantIndex) => {
      ...
      const rankings = games
        ...
        .filter(
          (ranking) =>
            ranking !== null && ranking !== undefined && !Number.isNaN(ranking),
        );
      ...
      {participantUserFields.map((participant) => (
        ...
        >
          <Text fontSize={["xl", "2xl"]}>
            {averageRankingByParticipant[
              participantUserFields.indexOf(participant)
            ] || "-"}
          </Text>
        </Td>
      ))}
