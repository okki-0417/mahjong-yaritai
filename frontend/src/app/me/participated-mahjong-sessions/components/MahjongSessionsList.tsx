...
type Session =
  ParticipatedMahjongSessionsQuery["participatedMahjongSessions"]["edges"][0]["node"];
...
export default function MahjongSessionsList({
  initialSessions,
  initialPageInfo,
}: Props) {
  ...
  const fetchMore = useCallback(async () => {
    ...
    try {
      const { mahjongSessions: nextMahjongSessions, pageInfo: newPageInfo } =
        await fetchParticipatedMahjongSessions({
